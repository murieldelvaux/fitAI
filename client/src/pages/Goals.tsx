...
import { computeBiometrics, getCalculatedStrategies } from '../lib/nutritionCalculator';
import { Card, CardHeader, CardTitle, CardDescription, CardContent, CardFooter } from '../components/ui/Card';
import { Input } from '../components/ui/Input';
import { Button } from '../components/ui/Button';
import { Badge } from '../components/ui/Badge';
import { BiometricProfileCard } from '../components/nutrition/BiometricProfileCard';
...
import { StrategyMacros } from '../types/biometrics';
...
export function Goals() {
  ...
  const calculatedStrategies = getCalculatedStrategies(profile, biometrics);
  ...
  const applyPreset = (preset: StrategyMacros) => {
    ...
    toast.info(`Estratégia "${preset.name}" aplicada. Clique em Salvar para confirmar.`);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-300 pb-12">
      ...
      {/* 1. Biometric Profile Section */}
      <BiometricProfileCard />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
          ...
          {biometrics.isComplete ? (
            <span className="text-xs text-green-400 font-semibold flex items-center gap-1">
              ...
              Valores calculados para seu perfil biométrico
            </span>
          ) : (
            <span className="text-xs text-slate-500">
              Valores padrão de referência (preencha a biometria acima para personalizar)
            </span>
          )}
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {calculatedStrategies.map((preset) => (
            <Card
              key={preset.key}
              className={`p-4 border cursor-pointer transition-all duration-200 bg-[#1E293B] group relative overflow-hidden ${
                preset.isCustomized
                  ? 'border-slate-800 hover:border-green-500/50 hover:shadow-lg hover:shadow-green-500/5'
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex items-center gap-2">
                  <div className="p-2 rounded-xl bg-slate-900 border border-slate-800 group-hover:border-green-500/30 transition-colors">
                    {getStrategyIcon(preset.key)}
                  </div>
                ...
                {preset.isCustomized && (
                  <Badge
                    variant="primary"
                    className="text-[10px] font-bold px-2 py-0.5 tracking-tight shrink-0 border-green-500/40"
                  >
                    Personalizado
                  </Badge>
                )}
              ...
              <p className="text-xs text-slate-400 leading-relaxed min-h-[36px]">{preset.desc}</p>

              <div className="mt-3 pt-2.5 border-t border-slate-800/80 flex justify-between text-xs font-semibold text-slate-300">
                <span className="font-bold text-white">{preset.calories} kcal</span>
      ...
      {/* 3. Main Goal Form */}
      <Card className="border-slate-800 bg-[#1E293B] shadow-lg">
        <CardHeader>
          <CardTitle>Personalizar Metas de Macronutrientes</CardTitle>
          <CardDescription>
            Insira os valores exatos desejados para cada indicador diário ou escolha uma das estratégias acima.
          </CardDescription>
        </CardHeader>
